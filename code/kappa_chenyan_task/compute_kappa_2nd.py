...
def main():
    ...
    # NOTE: this copy lives in code/kappa_chenyan_task/. The sealed answer key is
    # intentionally NOT copied here; it is read automatically from the sibling
    # code/kappa/ folder when Chen Yan runs the script AFTER submitting her labels.
    ap.add_argument("--second", default=os.path.join(HERE, "kappa_second_coder_chenyan_FILLED.csv"))
    ...
    try:
        ...
        log("=" * 70)
        log("SECOND-CODER FILE NOT FOUND: " + args.second)
        log("")
        log("This script does NOT fabricate the second coder's answers.")
        log("To compute kappa:")
        log("  1. Open kappa_second_coder_template_BLANK.csv and fill the 3 coding")
        log("     columns (design_type, IV_selection_strategy, reports_overlap_risk)")
        log("     from each study's full text, using D61_Supplementary_codebook_S1.md.")
        log("  2. Save it as kappa_second_coder_chenyan_FILLED.csv (same folder).")
